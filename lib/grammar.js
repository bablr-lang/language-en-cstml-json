 ...
 * @generatedfrom ./grammar.macro.js#769e60ce8f7f0e9a9efee9c6b6b050ccbccaceac
...
export const grammar = basicTriviaEnhancer({
  ...
  *String({
    ...
    yield q_ === "'" ? eat(m`openToken: <* "'" { balanced: "'", balancedSpan: 'String:Single' } />`) : eat(m`openToken: <* '"' { balanced: '"', balancedSpan: 'String:Double' } />`);
    yield eat(m`content$: <*StringContent />`);
    yield q_ === "'" ? eat(m`closeToken: <* "'" { balancer: true } />`) : eat(m`closeToken: <* '"' { balancer: true } />`);
  }
  *Property() {
    if (yield match(re`/['"]/`)) {
  ...
  *Identifier({
    ...
    if (pn) {
      q = yield eatMatch(m`openToken: <* ${buildString(pn)} { balanced: ${buildString(pn)} } />`, o({}), o({
        bind: true
      }));
    } else {
    ...
    if (q) {
      yield eat(m`closeToken: <* ${buildString(pn)} { balancer: true } />`, o({}), o({
        bind: true
      }));
    } else {
  ...
  *Infinity() {
    yield eatMatch(m`signToken: <* /[+-]/ />`, null, o({
      bind: true
    }));
